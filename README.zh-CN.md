...
[打开生产环境](https://catan-imitation.workers.dev)。进入页面后可选择“本地游戏”或“联机游戏”；界面默认使用英文，并可在“设置”中切换为简体中文。
...
本次发布会同时升级房间存储架构 v2 和联机协议 v2。Worker 与 SPA 必须作为同一次发布部署：兼容的 v1 大厅和与旧版固定棋盘完全一致的房间会自动迁移；不兼容的旧客户端会进入刷新/恢复路径，而不会收到不完整投影。
...
本项目将预览版本部署到独立的 `catan-imitation-preview` Worker，因为版本预览 URL 不适合该项目的 Durable Object 绑定。恢复生产流量时，只能切换到此前已验证且兼容存储架构 v2 的部署。任何房间迁移到架构 v2 后，都不得回滚到架构 v1 二进制；应切换到兼容的 v2 部署，或通过修正版本向前恢复。GitHub Pages 发布已退役，生产环境只保留一个主机；原 Pages 地址现返回 HTTP 404。
