...
| OM-009 | Origin-local room-keyed token storage; storage loss/cross-device recovery unsupported. | `test/online/onlineClient.test.ts` (origin-local namespace, room-code-only active pointer, stale/malformed cleanup, multi-tab reuse, unavailable/quota fallback); README files and `quickstart.md` state the limitation; `pnpm test`. | Pass |
...
| OM-012 | HTTPS token exchange yields a single-use approximately 30-second ticket. | `test/worker/security.test.ts` (exact 30-second expiry); `test/worker/roomLifecycle.test.ts` (single-use consumption plus 8-per-seat/32-per-room outstanding bounds and stable 429); `test/domain/smokeWorkerTicket.test.ts`; `pnpm test:worker`; `pnpm test`. | Pass |
...
| OM-019 | Reconnect returns a complete caller projection and version. | `test/online/onlineClient.test.ts` (active-session resolution and complete snapshot replacement); `test/online/onlineLobbyUi.test.ts` (injected resume without create/join); `test/e2e/online-multiplayer.spec.ts` (actual `page.reload()`, same seat/version/private view, fresh ticket, zero create/join); `pnpm test`; `pnpm test:e2e:online`. | Pass |
...
| OM-047 | Reconnect uses capped backoff and a fresh one-time ticket. | `test/online/onlineClient.test.ts` (1/2/4/8/15-second schedule and fresh ticket per socket); `test/e2e/online-multiplayer.spec.ts` (offline reconnect and actual page reload each request a new ticket and restore the same seat); `pnpm test`; `pnpm test:e2e:online`. | Pass |
...
| OM-054 | Protocol/message/resources/arrays/strings are bounded at network and domain boundaries. | `test/online/protocol.test.ts` (16 KiB and field bounds); `test/worker/security.test.ts` (HTTP byte limits); `test/worker/roomWebSocket.test.ts` (inbound/outbound limits); `test/worker/roomLifecycle.test.ts` (stored-ticket schema rejects over 8 per seat or 32 total); `pnpm test`; `pnpm test:worker`. | Pass |
...
| OM-056 | Per-seat burst throttling rejects abuse without corruption. | `test/worker/roomWebSocket.test.ts` (10 commands/2 seconds, exact boundary, unchanged state); `test/worker/roomLifecycle.test.ts` (10 authenticated ticket-issuance admissions/2 seconds, saturated store-level 429s are charged, later 429s stop before ticket crypto/storage, invalid credentials allocate no admission, ninth outstanding rejection leaves room unchanged); `pnpm test:worker`. | Pass |
...
| OM-059 | Stored/wire schema compatibility rejects unsafe versions explicitly. | `test/worker/roomLifecycle.test.ts` (incompatible/malformed storage, legacy-v1 expired-ticket normalization, still-live over-bound rejection without mutation, and reconstruction result); `test/online/protocol.test.ts` (wire schema); `test/online/onlineClient.test.ts` (terminal incompatible state); `pnpm test:worker`; `pnpm test`. | Pass |
...
- Client recovery: `test/online/onlineClient.test.ts`, `test/online/onlineLobbyUi.test.ts`, and `test/e2e/online-multiplayer.spec.ts` cover a room-code-only active pointer, boot-time route/session restoration, actual `page.reload()`, a fresh ticket, equal-or-newer complete caller-private state, explicit active-marker clearing, terminal states, and unsupported storage-clearing/cross-device recovery.
...
- Avoidable complexity: T023 reuses the credential store, lobby/router composition, room store, Durable Object route, and stable `RATE_LIMITED` error. It adds no dependency, persisted field, schema/migration version, service, router framework, or alternate credential path; the unused direct crypto-random factory was deleted.
